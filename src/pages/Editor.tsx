          ...
          <div className="lg:col-span-2">
            <Card className="mx-[70px]">
              <CardContent className="p-6 px-0 mx-[14px] py-[64px]">
                <div className="relative bg-white mx-auto overflow-hidden border" style={{
                ...
                height: `${currentDimensions.height / 2}px`
                  ...
                  {/* Background Image */}
                  {uploadedImage && <div className="absolute inset-0">
                  ...
                  <div className="absolute bottom-0 left-0 p-4 w-1/2">
                    {headlineText && <div className="text-white font-bold text-lg mb-1 text-shadow">
                        {headlineText}
                      </div>}
                    
                    {subheadlineText && <div className="text-white text-sm mb-3 text-shadow">
                        {subheadlineText}
                      </div>}
                    
                    {/* CTA Button */}
                    <button className={`px-3 py-1 text-xs rounded font-medium ${formData.market === 'cz' || formData.market === 'hu' || formData.market === 'at' ? 'cta-button-cz' : 'cta-button-de'}`}>
                      {ctaData?.text || 'Buy now >>'}
                    </button>
                  </div>
                  
                  {/* Price tag (right side) */}
                  {showPriceTag && <div className="absolute top-1/4 right-8">
            ...
            <Card className="mb-6">
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <Input id="headline" value={headlineText} onChange={e => setHeadlineText(e.target.value)} placeholder="Enter headline text" className="mt-2" />
                </div>
                
                <div>
                  <Label htmlFor="subheadline">Subheadline:</Label>
                  <Input id="subheadline" value={subheadlineText} onChange={e => setSubheadlineText(e.target.value)} placeholder="Enter subheadline text" className="mt-2" />
                </div>
                
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between mb-4">
                    <Label htmlFor="show-price">Show Price Tag (Preview Only):</Label>
                    <input id="show-price" type="checkbox" checked={showPriceTag} onChange={e => setShowPriceTag(e.target.checked)} className="h-4 w-4" />
                  </div>
                  
                  {showPriceTag && <div>
                      <Label htmlFor="price-value">Price Value:</Label>
                      <Input id="price-value" value={priceValue} onChange={e => setPriceValue(e.target.value)} placeholder="e.g. 99" className="mt-2" />
                      <p className="text-xs text-gray-500 mt-1">
