    ...
    <div className="space-y-4">
      ...
      {uploadedImages.length > 0 && (
        <div className="mt-4">
          ...
          <ScrollArea className="h-[200px] w-full border rounded-md p-2 mt-1">
            <div className="space-y-2">
              {uploadedImages.map((image, index) => (
                <div 
                  key={index} 
                  className={`flex items-center p-2 rounded ${activeImageIndex === index ? 'bg-gray-100' : ''}`}
                >
                  <div 
                    className="w-12 h-12 mr-3 flex-shrink-0 cursor-pointer border overflow-hidden"
