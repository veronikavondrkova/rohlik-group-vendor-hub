...
const BackgroundImage: React.FC<BackgroundImageProps> = ({
  ...
  if (uploadedImages.length > 0) {
    return <>
        {uploadedImages.map((image, index) => <Draggable key={index} position={image.position} onDrag={position => onDrag(index, position)} bounds="parent">
            <img src={image.src} alt={image.fileName} className="absolute cursor-move" style={{
          display: 'block',
          width: 'auto',
          height: 'auto',
